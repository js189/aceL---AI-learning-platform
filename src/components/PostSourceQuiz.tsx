...
export function PostSourceQuiz({
  ...
  async function loadQuiz(freshQuestions = false) {
    setApiError("");
    setLoading(true);
    try {
  ...
  async function assessQuiz() {
    ...
    try {
      ...
      if (currentQ + 1 >= questions.length) {
        ...
        await saveAttemptToSupabase(avgScore, qResults);
      ...
      <div className="mt-6 rounded-card border border-warm-sand/80 bg-cream p-4 sm:p-6 shadow-subtle">
        ...
        {q.type === "mcq" && q.options?.length ? (
          <ul className="mt-4 space-y-2">
            {q.options.map((opt, i) => (
              <li key={i}>
                <label className="flex cursor-pointer items-center gap-3 py-3 sm:py-2 px-4 min-h-[52px] sm:min-h-0 rounded-input border-2 border-warm-sand/50 has-[:checked]:border-dusty-blue has-[:checked]:bg-dusty-blue/5">
                  <input
                    type="radio"
                    name={q.id}
          ...
          <textarea
            ...
            rows={4}
            className="mt-4 w-full rounded-input border-2 border-warm-sand/80 px-4 py-3 text-deep-charcoal"
          />
        )}
        <button
          onClick={assessQuiz}
        ...
        >
          {assessing ? "Checking…" : currentQ + 1 >= questions.length ? "Finish" : "Next"}
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="mt-6 rounded-card border border-dusty-blue/30 bg-dusty-blue/5 p-4 sm:p-6">
      ...
      {apiError && (
        <p className="mt-2 text-sm text-terracotta">{apiError}</p>
      )}
      <button
      ...
      >
        {loading ? "Generating…" : "Take Post-Source Quiz"}
      </button>
    </div>
  );
}
